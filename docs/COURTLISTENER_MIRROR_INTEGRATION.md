...
- Endpoint shape: `POST /v1/authority/search` (consumer client only; mirror agent may expose an equivalent approved route)
- Contract version: `1`
...
`FixtureTransport` proves the contract offline. `HttpMirrorTransport` is HTTPS-only, bounded, redirect-rejecting, size-limited, and typed-error based. `CourtListenerMirrorProvider` is a thin facade over that transport. Database details, ranking, treatment, and currentness remain outside the provider adapter.
